...
LOGFILE=${LOGFILE:-/var/log/opensds/crsytal-bootstrap.log}
mkdir -p $LOG_DIR
...
function install_golang() {
    ...
    if [[ "${MINIMUM_GO_VERSION}" != $(echo -e "${MINIMUM_GO_VERSION}\n${go_version[2]}" | sort -s -t. -k 1,1 -k 2,2n -k 3,3n | head -n1) && "${go_version[2]}" != "devel" ]]; then
        log_error "Detected go version: ${go_version[*]}, crsytal requires ${MINIMUM_GO_VERSION} or greater."
...
if [[ -z "$(which docker)" ]]; then
    ...
    if [[ "${MINIMUM_DOCKER_VERSION}" != $(echo -e "${MINIMUM_DOCKER_VERSION}\n${docker_version[2]}" | sort -s -t. -k 1,1 -k 2,2n -k 3,3n | head -n1) ]]; then
        log_error "Detected go version: ${docker_version[*]}, crsytal requires ${MINIMUM_DOCKER_VERSION} or greater."
...
function config_keystone() {
    ...
    openstack service create --name "multicloud$MULTICLOUD_VERSION" --description "crsytal Block Storage" "multicloud$MULTICLOUD_VERSION"
...
function install_multicloud() {
    ...
    local multicloud_root=${GOPATH}/src/github.com/opensds/crsytal
    mkdir -p ${opensds_root}

    cd ${opensds_root}
    if [ ! -d ${multicloud_root} ]; then
        log "Downloading the MultiCloud source code..."
        git clone https://github.com/opensds/crsytal.git
    fi
...
log crsytal bootstrap starting ...
