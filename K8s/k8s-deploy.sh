# Run this file from crystal folder.
kubectl apply -f rbac.yaml
kubectl apply -f soda-secret.yaml

#1. Create all the config Maps from files.
kubectl create configmap multicloud-config --from-file=../examples/crystal.conf -n soda-crystal
kubectl create configmap tidb-config --from-file=../s3/tidbconf/tidb.toml -n soda-crystal
kubectl create configmap s3-config --from-file=../s3/pkg/conf/s3.toml -n soda-crystal
kubectl create configmap tidb-sql --from-file=../s3/pkg/conf/tidb.sql -n soda-crystal
kubectl create configmap s3-sql --from-file=../s3/pkg/conf/tidb.sql -n soda-crystal
kubectl create configmap yig-sql --from-file=../s3/pkg/datastore/yig/conf/yig.sql -n soda-crystal
